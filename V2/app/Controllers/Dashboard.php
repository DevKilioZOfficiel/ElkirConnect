	...
	public function sendproject(){
		...
		if (!$this->validate($rules)) {
			return redirect()->to('dashboard/new')->withInput()->with('errors', $this->validator->getErrors());
				...
				$builder->insert($data);
				return redirect()->to('dashboard/new')->withInput()->with('success', "Votre projet est actuellement en attente. Une réponse sous peu sera envoyée par email !")->with('success__project_name', strip_tags($this->request->getVar('project_name')));
		}
	}
	//--------------------------------------------------------------------
